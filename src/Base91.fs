...
type Base91 private ( configuration : BinaryToTextConfiguration ) =
    ...
    /// Encodes a byte array into a Base91 string. Optionally wrap output at specified column; for efficiency, column must be a multiple of 4, will be rounded down if necessary. Throws exception on invalid input.
